...
**A window that does not resolve is NOT a state.** `pane-state.sh` exits
**3** with `no such tmux window: <idx>` on stderr and prints no `state=`
line at all (issue `#140`). This is deliberate and must not be "simplified"
into reporting `absent`: old tmux writes its lookup error to stdout, while
tmux 3.x silently falls back to the session's **active** window and returns
a confident, valid classification of the **wrong** window. Comparing the
resolved index against the requested one catches both modes; refusing to
answer is the only safe response, since a bare `absent` here is
indistinguishable from a typo'd index and would have the orchestrator
relaunch or close on the strength of it.

So the two conditions are asked of different tools:

- *"Has claude died in this worker?"* → `pane-state.sh` → `state=absent`
  (the window is still there, held open by `remain-on-exit on`).
- *"Has the window gone away?"* → `tmux list-windows`, never `pane-state.sh`.

Two integration scenarios asserted `state=absent` after `tmux kill-window`
and were red for it; the failure was read as a tmux-version limitation
until `<your-org>/nexus-code#568` traced it to this distinction. Both now
assert the pair separately.
