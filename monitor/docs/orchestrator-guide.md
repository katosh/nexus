...
| `monitor/pane-state.sh <window-key>` | Render-proof pane classification. The key is an index, a `session:window`, or a window NAME. **`monitor/pane-state.sh --states` IS the vocabulary — ask it rather than copying a list from here**; a stale copy that omits `working-background` / `working-self-paced` retires a live worker. Never eyeball `tmux capture-pane` — autosuggest ghost-text is indistinguishable from typed input in plain text. | [`docs/reference/worker-states.md`](../../docs/reference/worker-states.md) |
