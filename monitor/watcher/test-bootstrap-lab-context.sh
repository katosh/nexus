...
# _NEXUS_HOSTNAME, HOME, SANDBOX_PROJECT_DIR) so the captured prompt's
# yes/no values are deterministic and we can assert both shapes.
#
...
run_bootstrap() {
    local home="$1" hostname="$2" hpc_mount="$3" sandbox="$4"
    : > "$CAPTURE"
    env -i \
        PATH="/usr/bin:/bin" \
...
run_bootstrap "$HOME_WITH_SKILLS" "loginnode" "$HPC_MOUNT" "$SBX_WITH_LABSH"
...
run_bootstrap "$HOME_NO_SKILLS" "laptop-1234" "$WORK/no-such-shared" "$SBX_NO_LABSH"
assert_eq "bootstrap exit code is 0" "$LAST_RC" "0"
